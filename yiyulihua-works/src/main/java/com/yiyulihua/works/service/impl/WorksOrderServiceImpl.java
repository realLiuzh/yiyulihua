...
import com.yiyulihua.works.dao.WorksOrderDao;
...
public class WorksOrderServiceImpl extends ServiceImpl<WorksOrderDao, WorksOrderEntity> implements WorksOrderService {
    ...
    @Autowired
    public WorksOrderServiceImpl(WorksService worksService) {
    ...
    public String createOrder(OrderTo order) {
        ...
        //更新作品的报价人数
        WorksEntity worksEntity = worksService.getById(order.getWorksId());
        worksEntity.setWorksBidNumber(worksEntity.getWorksBidNumber() + 1);
        boolean updateById = worksService.updateById(worksEntity);
        AssertUtil.isTrue(!updateById, new ApiException(ApiExceptionEnum.INTERNAL_SERVER_ERROR));
